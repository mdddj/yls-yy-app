...
- 每隔 N 秒轮询接口（可配置）
- 支持配置多个 `Codex API Key` 账号并同时监控，旧版单个 Key 自动迁移为第一个账号
- 可在菜单中配置 `AGI API Key`
- 可在菜单中切换统计模式：`单显` / `双显`
- 可在菜单中切换单显时的展示数据源（`Codex` / `AGI`）
- 双显模式会同时显示 `Codex` 和 `AGI` 两组统计，并支持点击分组展开/收起
- 主界面（单显/双显）会列出全部 Codex 账号的额度统计，点击即可切换状态栏显示的账号
- 每个账号行带「切换」按钮，一键将该账号 Key 写入 `~/.codex/auth.json`（`OPENAI_API_KEY`），本地 Codex 立即切换使用该账号；本地正在使用的账号以绿色图标标识
- 多 Codex 账号时可在「Codex 账号」管理窗口中为每个账号命名、删除，并选择状态栏显示哪一个账号
...
- `选择统计模式...`
- `选择单显套餐源...`
- `Codex 账号`（添加/删除/命名多个 Codex Key，并选择状态栏显示的账号）
- `设置 AGI API Key...`
- `设置轮询间隔...`
- `立即刷新`
